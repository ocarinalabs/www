    ...
    const renderFrame = () => {
      ...
      if (!wireframe) {
        ...
        for (let face = 0; face < 6; face++) {
          ...
          const nz =
            -ni * Math.sin(B) +
            nj * Math.sin(A) * Math.cos(B) +
            nk * Math.cos(A) * Math.cos(B);
          if (backfaceCulling && nz > 0) continue;
          const ch = FACE_CHARS[face];
          const colorVal = color
            ? FACE_COLORS[face % FACE_COLORS.length]
            : "currentColor";
      ...
      for (let y = 0; y < H; y++) {
        ...
        for (let x = 0; x < W; x++) {
          ...
          if (chbuf[idx] !== " ") {
            line.push(
              <span key={x} style={{ color: cbuf[idx], fontWeight: "bold" }}>
                {chbuf[idx]}
              </span>
            );
          } else {
